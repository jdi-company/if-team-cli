...
- **Bracket-notation filters**: NestJS DTOs use form-array notation. Pass keys like `'filter[status_id][]': '5'` directly to `apiRequest`'s `query` option — `URLSearchParams.set()` URL-encodes them correctly. On `GET /tasks` the live API silently ignores direct query params (e.g. `project_id=5` returns the full company) and only honors the `filter[…][]` form — always serialize list filters as `filter[<field>][]`, even for scalar single-value filters like project or status. Date filters are an exception: they take a 2-element indexed tuple `filter[<field>][0]=…&filter[<field>][1]=…` (from/to).
